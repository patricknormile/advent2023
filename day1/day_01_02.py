callibration_code = """heightseven4two5
npskfdstpk2knsm
...
def find_first_digit(x, reverse=False) : 
    base_digits = [str(z) for z in range(1,10)]
    word_digits = ["one","two","three","four","five",
                   "six","seven","eight","nine"]
    value_lookup = dict(zip(word_digits,base_digits))
    digits = base_digits + word_digits
    n = len(x) 
    for i in range(1,n+1) : 
        if x[(i-1):i] in base_digits : 
            return x[(i-1):i]
        else : 
            for j in range(i) :
                value = x[j:i]
                if reverse : 
                    value = value[::-1]     
                if value in word_digits : 
                    return value_lookup[value]
                

def first_dig_last_dig(x) : 
    
    dig0 = find_first_digit(x)
    dign = find_first_digit(x[::-1], reverse=True)
    try : 
        out = int(dig0 + dign)
    except : 
