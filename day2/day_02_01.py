data = """Game 1: 7 green, 14 red, 5 blue; 8 red, 4 green; 6 green, 18 red, 9 blue
...
def parse_row_number(row) : 
    number = int(re.sub("[^(Game \d{1,2})].+",'',row)[5:])
    return number

def parse_game_outcome(row) : 
    games = re.sub("Game \d{1,3}: ","",row).split("; ")
    draws = [x for game in games for x in game.split(", ")]
    out = {'red':0,'green':0,'blue':0}
    for x in draws : 
        if re.match('\d+ red',x) is not None : 
            new_red = int(re.sub(' red','', x))
            if new_red > out['red'] : 
                out['red'] = new_red
        elif re.match('\d+ green',x) is not None : 
            new_green = int(re.sub(' green','',x))
            if new_green > out['green'] : 
                out['green'] = new_green
        elif re.match('\d+ blue',x) is not None : 
            new_blue = int(re.sub(' blue','',x))
            if new_blue > out['blue'] : 
                out['blue'] = new_blue
        else : 
            raise ValueError(f"didn't make it {x}")
    return out

def evaluate_game(row) : 
    game_dict = parse_game_outcome(row)
    if game_dict['red'] > 12 : 
        return False
...
if __name__ == '__main__' : 
    ...
    ids = [parse_row_number(r) for r in data_rows 
    if evaluate_game(r)]
    print(sum(ids))
